  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        // Fetch user events
        const eventsResponse = await axios.get("https://event-management-system-backend-dtrl.onrender.com/events-registered", {
          headers: { Authorization: `Bearer ${token}` },
        });
...
<span className="loading loading-dots loading-sm"></span>  Loading profile...</p>
  ...
  {events.length > 0 ? (
    ...
    <p className="text-gray-700">😢No events registered yet.</p>
  )}
</div>
